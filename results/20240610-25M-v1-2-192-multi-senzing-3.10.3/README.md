# senzing-test-results-20240610-25M-v1-2-192-multi-senzing-3.10.3
...
1. Inserts per second:
    1. Peak: 5309/second
    1. Warm-up: 25 mins
    1. Average after warm-up: 4492/second
    1. Average over entire run: 3530/second
    1. Time to load 25M: 1.7 hours
    1. Records in dead-letter queue: 0
    1. Volume read IOPS:       508,216
    1. Volume write IOPS:  100,824,689
    1. See [dsrc_record.csv](data/dsrc_record.csv)

Note:  Withinfo disabled.

- Max Stream-loader tasks: 100
- Max Redoer tasks: 76
...
G2=> SELECT NOW(), COUNT(*) FROM DSRC_RECORD;
              now              |  count
-------------------------------+----------
 2024-06-10 19:51:49.440559+00 | 21645223
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM OBS_ENT;
              now              |  count
-------------------------------+----------
 2024-06-10 19:51:54.592125+00 | 21112312
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT;
              now              |  count
-------------------------------+----------
 2024-06-10 19:51:57.765811+00 | 18024268
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT_OKEY;
              now              |  count
-------------------------------+----------
 2024-06-10 19:52:07.505032+00 | 21112312
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM SYS_EVAL_QUEUE;
              now              | count
-------------------------------+-------
 2024-06-10 19:52:11.638042+00 |     0
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_RELATE;
              now              |  count
-------------------------------+----------
 2024-06-10 19:52:15.065552+00 | 10490010
(1 row)

G2=> select min(first_seen_dt) load_start, count(*) / (extract(EPOCH FROM (max(first_seen_dt)-min(first_seen_dt)))/60) erpm, count(*) total, max(first_seen_dt)-min(first_seen_dt) duration, (count(*) / (extract(EPOCH FROM (max(first_seen_dt)-min(first_seen_dt)))/60))/60 as avg_erps from dsrc_record;
       load_start        |        erpm        |  total   |   duration   |      avg_erps
-------------------------+--------------------+----------+--------------+--------------------
 2024-06-10 17:43:41.301 | 211826.95317297586 | 21645223 | 01:42:11.011 | 3530.4492195495977
(1 row)
