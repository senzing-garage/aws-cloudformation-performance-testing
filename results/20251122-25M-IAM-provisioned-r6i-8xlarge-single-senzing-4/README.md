# senzing-test-results--25M-IAM-provisioned-r6i-8xlarge-single-senzing-4
...
1. Performed: Nov 22, 2025
2. Senzing version: 4.2.0.25325
3. Instructions:
...
4. Changes:
    ...
    1. Loader(consumer) and redoer both use IAM
...
1. Inserts per second:
    ...
    1. Average over entire run: 1726/second
    1. Time to load 25M: 4.02 hours
    1. Records in dead-letter queue: 0
    1. Volume read IOPS:      1,905,801
    1. Volume write IOPS:   117,781,128
...
G2=> SELECT NOW(), COUNT(*) FROM DSRC_RECORD;
            now             |  count
----------------------------+----------
 2025-11-22 21:26:09.362+00 | 25000000
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM OBS_ENT;
              now              |  count
-------------------------------+----------
 2025-11-22 21:26:14.035242+00 | 24999937
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT;
              now              |  count
-------------------------------+----------
 2025-11-22 21:26:20.546418+00 | 21133755
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT_OKEY;
              now              |  count
-------------------------------+----------
 2025-11-22 21:26:24.913789+00 | 24999937
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM SYS_EVAL_QUEUE;
              now              | count
-------------------------------+-------
 2025-11-22 21:26:29.076492+00 |     0
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT WHERE ent_state != 0 ;
             now              | count
------------------------------+-------
 2025-11-22 21:26:32.56607+00 |    47
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_RELATE;
              now              |  count
-------------------------------+----------
 2025-11-22 21:26:37.929865+00 | 11262442
(1 row)

G2=> select min(first_seen_dt) load_start, count(*) / (extract(EPOCH FROM (max(first_seen_dt)-min(first_seen_dt)))/60) erpm, count(*) total, max(first_seen_dt)-min(first_seen_dt) duration, (count(*) / (extract(EPOCH FROM (max(first_seen_dt)-min(first_seen_dt)))/60))/60 as avg_erps from dsrc_record;
       load_start        |          erpm           |  total   |   duration   |       avg_erps
-------------------------+-------------------------+----------+--------------+-----------------------
 2025-11-22 16:27:14.764 | 103545.5872908974523851 | 25000000 | 04:01:26.373 | 1725.7597881816242064
(1 row)
