# senzing-test-results-20250307-100M-AMD-provisioned-r6i-8xlarge-single-senzing-4
...
1. Performed: Mar 07, 2025
2. Senzing version: 4.0.0.25065
3. Instructions:
...
1. Inserts per second:
    ...
    1. Average over entire run: 788/second
    1. Time to load 100M: 35.25 hours
    1. Records in dead-letter queue: 0
    1. Volume read IOPS:    205,865,075
    1. Volume write IOPS:   606,453,122
...
G2=> SELECT NOW(), COUNT(*) FROM DSRC_RECORD;
              now              |   count
-------------------------------+-----------
 2025-03-09 02:30:29.534141+00 | 100000000
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM OBS_ENT;
             now              |  count
------------------------------+----------
 2025-03-09 02:33:05.61993+00 | 99998927
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT;
              now              |  count
-------------------------------+----------
 2025-03-09 02:36:35.068304+00 | 61001365
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_ENT_OKEY;
             now              |  count
------------------------------+----------
 2025-03-09 02:37:09.61118+00 | 99998927
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM SYS_EVAL_QUEUE;
              now              | count
-------------------------------+-------
 2025-03-09 02:37:26.306622+00 |     0
(1 row)

G2=> SELECT NOW(), COUNT(*) FROM RES_RELATE;
              now              |  count
-------------------------------+----------
 2025-03-09 02:37:30.835613+00 | 31815374
(1 row)

G2=> select min(first_seen_dt) load_start, count(*) / (extract(EPOCH FROM (max(first_seen_dt)-min(first_seen_dt)))/60) erpm, count(*) total, max(first_seen_dt)-min(first_seen_dt) duration, (count(*) / (extract(EPOCH FROM (max(first_seen_dt)-min(first_seen_dt)))/60))/60 as avg_erps from dsrc_record;
       load_start        |          erpm          |   total   |      duration      |       avg_erps
-------------------------+------------------------+-----------+--------------------+----------------------
 2025-03-07 04:45:42.866 | 47261.5916969953490348 | 100000000 | 1 day 11:15:52.982 | 787.6931949499224839
(1 row)
